"""
This module provides the functions needed to apply post-processing to the CNN output.
"""
...
def apply_postprocessing(img, dictionary, svm):
    ...
    img = resize(img, (img.shape[0] // const.POSTPRO_PATCH_SIZE, img.shape[1] // const.POSTPRO_PATCH_SIZE), order=0,
                 preserve_range=True)
        ...
        for i in range(1, len(images) + 1):
            ...
            for x in range(0, num_rows):
                for y in range(0, num_cols):
                    id = str(i).zfill(3) + "_" + str(const.POSTPRO_PATCH_SIZE * x) + "_" +\
                         str(const.POSTPRO_PATCH_SIZE * y)
...
def generate_output():  
    """Applies post-processing to the CNN output. Output predictions are saved as images and submission.csv is also
    created."""
...
#    input_filename = "raw_test_%d_patches"
#    output_dir = postpro_fn + "/test/"
#    num_images = 50    
    
    # training set
#    prob_fn = "../results/CNN_Output/training/high_res_raw/"  
#    input_filename = "raw_satImage_%.3d_pixels"
#    output_dir = postpro_fn + "/training/"
#    num_images = 100
    ...
    for i in range(1, num_images+1):
        ...
        if os.path.isfile(image_filename):
            ...
            output_images.append(apply_postprocessing(img, D, svm))
            scipy.misc.imsave(output_dir + ("satImage_%d" % i) + ".png",
                              resize(output_images[i - 1], img.shape, order=0, preserve_range=True))
        else:
            print('File ' + image_filename + ' does not exist')
            
    create_submission_file(output_images)
