...
tf.app.flags.DEFINE_string('train_dir', 'baseline/tmp',
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    with tf.Session() as s:
        ...
        if not os.path.isdir(test_prediction_dir):
            os.mkdir(test_prediction_dir)

        def get_prediction_test(image_idx, test_data_filename):
            """Get test prediction."""
        ...
        with open('../results/baseline_submission.csv', 'w') as csvfile:
